...
document.getElementById('file-personal-photo').addEventListener('change', function(e) {
...
document.getElementById('infopersonnel').querySelectorAll('input, textarea').forEach(input => {
    input.addEventListener('input', handlePersonalInfoChange);
});

// Fonction pour ajouter une expérience professionnelle
document.getElementById('button-add-experiencee').addEventListener('click', function() {
    const poste = document.getElementById('input-experience-jobtitle').value;
    const entreprise = document.getElementById('input-experience-company').value;
    ...
    const description = document.getElementById('textarea-experience-description').value;
    
    if (poste && entreprise) {
        // Formatage des dates pour affichage
        const debutFormatted = new Date(debut).toLocaleDateString('fr-FR', {year: 'numeric', month: 'short'});
        const finFormatted = fin ? new Date(fin).toLocaleDateString('fr-FR', {year: 'numeric', month: 'short'}) : 'Présent';
        ...
        experienceList.innerHTML = `
            <div class="flex justify-between">
                ...
                <button class="delete-experience text-red-500 hover:text-red-700" data-index="${cvData.experiences.length - 1}">
...
document.getElementById('button-add-education').addEventListener('click', function() {
    const diplome = document.querySelector('[name="input-education-degree"]').value;
    const ecole = document.querySelector('[name="input-education-school"]').value;
    const debut = document.querySelector('[name="input-education-startdate"]').value;
    const fin = document.querySelector('[name="input-education-enddate"]').value;
    ...
    if (diplome && ecole) {
        ...
        formationList.innerHTML = `
            <div class="flex justify-between">
                ...
                <button class="delete-formation text-red-500 hover:text-red-700" data-index="${cvData.formations.length - 1}">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
...
document.getElementById('button-add-skill').addEventListener('click', function() {
    ...
    if (skill) {
        ...
        skillList.innerHTML = `
            <span>${skill}</span>
            <button class="delete-skill text-red-500 hover:text-red-700" data-index="${cvData.competences.length - 1}">
                <i class="fas fa-trash"></i>
            </button>
        `;
...
document.getElementById('button-add-interest').addEventListener('click', function() {
    const interest = document.querySelector('[name="input-interest-name"]').value;
    ...
    if (interest) {
        ...
        interestList.innerHTML = `
            <span>${interest}</span>
            <button class="delete-interest text-red-500 hover:text-red-700" data-index="${cvData.interets.length - 1}">
                <i class="fas fa-trash"></i>
            </button>
        `;
...
document.getElementById('button-add-reference').addEventListener('click', function() {
    ...
    if (nom && entreprise) {
        ...
        referenceList.innerHTML = `
            <div class="flex justify-between">
                ...
                <button class="delete-reference text-red-500 hover:text-red-700" data-index="${cvData.references.length - 1}">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
...
document.getElementById('button-add-language').addEventListener('click', function() {
    ...
    if (langue && langue !== 'Choisissez') {
        ...
        languageList.innerHTML = `
            <span>${langue}</span>
            <button class="delete-language text-red-500 hover:text-red-700" data-index="${cvData.langues.length - 1}">
                <i class="fas fa-trash"></i>
            </button>
        `;
...
document.body.addEventListener('click', function(e) {
    if (e.target.classList.contains('delete-skill') || e.target.parentElement.classList.contains('delete-skill')) {
        const button = e.target.classList.contains('delete-skill') ? e.target : e.target.parentElement;
        const index = button.dataset.index;
        cvData.competences.splice(index, 1);
        button.closest('div').remove();
        updatePreview();
    } else if (e.target.classList.contains('delete-experience') || e.target.parentElement.classList.contains('delete-experience')) {
        const button = e.target.classList.contains('delete-experience') ? e.target : e.target.parentElement;
        const index = button.dataset.index;
        cvData.experiences.splice(index, 1);
        button.closest('div.mt-2').remove();
        updatePreview();
    } else if (e.target.classList.contains('delete-formation') || e.target.parentElement.classList.contains('delete-formation')) {
        const button = e.target.classList.contains('delete-formation') ? e.target : e.target.parentElement;
        const index = button.dataset.index;
        cvData.formations.splice(index, 1);
        button.closest('div.mt-2').remove();
        updatePreview();
    } else if (e.target.classList.contains('delete-interest') || e.target.parentElement.classList.contains('delete-interest')) {
        const button = e.target.classList.contains('delete-interest') ? e.target : e.target.parentElement;
        const index = button.dataset.index;
        cvData.interets.splice(index, 1);
        button.closest('div.mt-2').remove();
        updatePreview();
    } else if (e.target.classList.contains('delete-language') || e.target.parentElement.classList.contains('delete-language')) {
        const button = e.target.classList.contains('delete-language') ? e.target : e.target.parentElement;
        const index = button.dataset.index;
        cvData.langues.splice(index, 1);
        button.closest('div.mt-2').remove();
        updatePreview();
    } else if (e.target.classList.contains('delete-reference') || e.target.parentElement.classList.contains('delete-reference')) {
        const button = e.target.classList.contains('delete-reference') ? e.target : e.target.parentElement;
        const index = button.dataset.index;
...
document.getElementById('resetForm')?.addEventListener('click', function() {
    if (confirm('Êtes-vous sûr de vouloir réinitialiser le formulaire ? Toutes les données seront perdues.')) {
        ...
        document.querySelectorAll('.mt-2.p-2.bg-blue-50').forEach(el => el.remove());
